...
- 连续对话保留最多六个来源指针并在下轮重新读取，不复用旧生成回答作为证据。
- 持久记忆完整来源关系在原文展示配额之外核验；回答前后只复核实际选中的记忆和来源。
  机器人、人类及未知来源角色贯穿 Reader、证书与主模型简报。
- 整理批次以单一事务提交图、覆盖与来源进度，索引随后刷新；取消会等待数据库工作完成。
  索引失败显式记录，独立自动索引修复仍未实现。
...
- Add a private-input local-semantics A/B harness with self-contained reports and
  complete per-call token accounting. Personal examples and answer rubrics are
