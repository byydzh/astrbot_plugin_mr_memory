...
状态：2026-09-19，两小时开发阶段的结果，尚未上线。分支 `codex/mr-memory-redesign`，起点 `8478adf`。统一记忆图、主动导航、前后台共同写入、回忆活动、主意识输入目录与控制台已接入调用路径。真实模型调用已使用新的读取和保存路径；持续理解与实际回复质量尚未达到可宣称完成的程度。

## 这次实际完成了什么

旧 episode / semantic / topic / 实体图的独立运行时读写已经移除，原始数据表只由一次迁移读取。`memory_graph.py` 保存当前对象、可寻址连接、版本、依据变化、线索导航及回忆记录；`memory_migration.py` 负责导入旧数据。原文、群成员平台记录、用量与任务进度仍由 `store.py` 持有。

后续思路也成为 `reflection` 记忆，其工作时间和状态只是同一对象上的调度字段。它可以连接经历，也可以成为另一项认识的依据；关注列表与图读取不再各持一份不同的正文。回忆记录包含实际用到的对象版本、原运行地址和共同唤起；已交付活动目录与模型明确重新思考过分别记录。没有新活动时，不会因为模型暂时留下一个问题就每次调度重复付费。

`memory_protocol.py` 统一模型可用的理解和读写能力；`request_context.py` 让潜意识按需读取主意识在 MR 钩子时已有的输入和工具。前台和后台均可通过工具写入，前台最终 JSON 也可同时提交 `items`，减少单纯保存所需的往返。后台工具表在预算收尾期间保持稳定，真实原文和未完成问题仍有继续读取的入口。

控制台沿真实的记忆地址浏览经历、认识、连接和历史；新高层对象可以点击返回下层经历。已有配置、额度、时间窗口、主会话修复入口保留。共享协议、存储、请求快照已经分离，**后台调度仍在 `main.py` 中，没有完成原计划中的进一步拆分**。

## 实际观察及其边界

验证使用部署环境的原生 AstrBot provider 和已配置的 DS API 别名、模型参数、私有数据库副本。Harrier 使用相同权重和主要库版本在本地编码，以免另起模型争用线上内存；调用经过私有传输桥。实际向量读取成功返回经历、认识、主题、关联和线索候选。它验证索引仍可访问，不能作为线上向量延迟测量。

| 本次观测 | 得到的结果 | 不能由此推断 |
| --- | --- | --- |
| 两批各 500 条原始材料的后台处理 | 约 98 秒 / 324,274 Token；约 257 秒 / 253,162 Token，均有真实写入及处理进度 | 不是相同输入与相同代码的 A/B，不能计算优化收益率，也不能说线上积压已经减少 |
| 保留工具表的第二批续接 | 一次 121,825 输入 Token 中有 104,192 命中缓存 | 缓存改善不等于推理更快，思考输出仍可能主导延迟与费用 |
| 最后版本的三条前台调用 | 约 6.08、3.78、52.12 秒；最后一条检索后在最终输出中保存了一项认识 | 接口返回完成不证明认识正确；仍存在过度概括及给主意识规定答法的输出 |
| 存储、迁移、主请求保留、任务续接和控制台检查 | 对应改动的契约检查通过，界面能往返浏览关联内容 | 合成返回只验证程序行为，不能证明模型会自发形成、使用并修订高层理解 |

历史问题按时间抽取，再用开发副本运行；副本含问题发生后形成的认识，实际输出也出现了后来事实。这些调用是当前记忆状态下的运行观察，**不作为历史环境复现或线上质量前后对照**。本轮没有重新执行完整 Poe 主意识、图片理解及其外部工具，也没有证明最终群回复已改变。真实语料与逐次结果均留在仓库外。

## 仍未跨过的产品关口

可连接的记忆网络使高层理解成为可能，但可达性本身不会产生抽象、类比或正确的归因。本轮模型确实保存过跨经历的归纳，仍有把局部表现扩成长期判断的情况；未观察到足以证明稳定收益的自主重温、反复修订和后续使用。对此不能靠固定重温次数、按频率强化边或针对几句用户问题补规则来宣称解决。

下一步应贯通**共同经历 → 当前认识 → 后续行为 → 再理解**：让短期思路与同一图中的持久认识持续衔接，观察模型何时选择保留、引用和改变它们，并检查主意识是否真正采用改变后的含义。当前短期笔记仍是独立的延续文本，变化目录能提供入口，却不能保证模型把两者整合好。这是尚未完成的架构与模型协作问题。

同样，后台可完成一个批次不等于长期产能足够。需在连续真实交流中同时观察有用理解的变化、旧认识是否减少重复工作，以及同一资源配置下的净积压变化。当前证据不足以合并替换线上版本。
...
接回多粒度主动导航：**线索 → 方面 → 内容；内容 → 新线索与方面；主题或高层认识 → 具体经历**。词法、向量检索提供入口，读到的内容还能引出新的搜索方向。[MRAgent](https://arxiv.org/html/2606.06036v1) 为这种主动访问和多粒度结构提供研究依据；本文对共享写入、变化传播的选择是 MR 的产品设计，不是论文的既有证明，也不是已完成的论文复现。
