...
> **状态：研究初稿，2026-08-17。** 本文把已经复核的线上账本、人工证据检索集、单案例三臂 Provider 诊断与尚未完成的确认性实验严格分开。表中 `TBD` 表示数据尚未产生；它不是零、失败或可由单案例外推的数值。
...
既有高级语义和反馈案例不能承担总体结论。“好女孩”实验的旧评分没有要求模型识别其作为侮辱性讳称及被群友再次玩笑化的语义，因此高分主要反映主题聚合，构念效度不足；反馈实验只有两个手选行为样本，其中一个存在 control 天花板；真实调用 #726 的旧 r4 又把一条研究者重新标注为“支持带保留的口嫌体正直”的时序证据链判为完全无依据。围绕该开发案例完成的 A1/cache、B0/B16 与 A2/full-MR 三臂诊断共使用 10 次 Provider 调用、139,870 个已测 Token：三臂均覆盖必要证据组，但 A2 以 B16 的 11.93 倍 Token、3.49 倍受控总时延扩大了证据面，最终 brief 却没有形成更直接的群内释义。该案例已被反复用于开发，且没有独立盲评、重复或主 LLM 最终回答，因此只能定位为“搜索变深但 synthesis 未获证实”的失败诊断，不能预报总体胜负。当前最可靠的结论仍是：本地检索已经足以生成较强候选，B0 仍是未获质量验证的不及格弱基线；实时重建相对缓存的收益、代价与适用边界必须由未污染 holdout、盲评和配对实验决定。
...
本文不声称已经完整复现 MRAgent [2]，也不声称当前插件已具备合格的群聊语义理解。原论文公开 benchmark 尚未复跑；A1/B16/A2 只完成了一个污染开发案例的协议诊断，A3、主 LLM 最终回答盲评、重复实验与确认集均尚未完成。
...
### 5.7 单案例三臂诊断已完成，确认性 pilot 尚未完成

当前第一阶段 harness 覆盖同一固定候选集上的 A1/cache、B0/B16 与 A2/full-MR；C0、R0、A3、主 LLM 最终回答盲评和多调用确认集尚未实现或运行。围绕一个已被多轮开发使用的 researcher-labeled 调用，三臂各运行一次且均完成：cache 不调用 Provider；B16 调用一次；full-MR 进行八轮 agent 调用后因步数耗尽再做一次强制合成。全部运行共 10 次 Provider 调用、139,870 个已测 Token，usage ledger 完整且没有未知 usage。

三臂的自动 `required_group_recall` 都是 1.0，但这只说明最终 brief 在每个预注册证据组中至少引用了一条 required source，不是语义正确率。cache 把 38 条预取来源全部写入 brief，gold-key precision lower bound 仅 0.1579；B16 从相同 38 条来源中引用 6 条，lower bound 为 1.0；full-MR 访问 62 条、引用 10 条，lower bound 为 0.7。研究者与独立 agent 复核均认为：B16 给出了最直接且校准的群内解释，但漏掉了后续实际游玩证据；full-MR 找到了该证据，却没有把跨情节链闭合成目标释义，并把合理的“不要把购买写成逐字预购”扩大为对整个事件连接的疑虑。该复核知道 arm 标签，也不是独立人类盲评，所以三个 arm 的语义状态仍为 `PENDING_BLIND_HUMAN_REVIEW`。

**表 5　单个污染开发调用的三臂协议诊断**

| Arm | n | Model calls | Tool calls | Visited / cited sources | Required-group recall | Gold-key precision LB | 已测 Token | 受控总时延 | 研究者诊断（非盲评） |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| A1：local materialization | 1 | 0 | 0 | 38 / 38 | 1.0 | 0.1579 | 0 | 0.001 s | 只有材料堆叠；未形成语义判断或必要疑虑 |
| B0/B16：single gate | 1 | 1 | 0 | 38 / 6 | 1.0 | 1.0000 | 10,820 | 21.094 s | 目标释义直接且有保留；缺后续游玩证据 |
| A2：full MR | 1 | 9 | 29 | 62 / 10 | 1.0 | 0.7000 | 129,050 | 73.560 s | 找到更多跨情节证据；最终 synthesis 未闭合语义 |

Gold-key precision lower bound 只计算引用 source key 与预注册 gold key 的重合下界，不是 Supported-Claim Precision。单次时延没有 p50/p95；cache 的 1.277 ms 只是本地物化器运行时间，B16/A2 是非流式直连接口的完整响应时间，二者都不是 AstrBot 线上端到端或首块时延。full-MR 的 29 次工具调用中有 18 次没有返回 source key，最后连续 6 次也没有新增可审计来源；本地工具执行总计约 14 ms，主要成本来自多轮模型调用和不断增长的上下文。隐私安全聚合见 [`pilot_provider_smoke_aggregate.json`](data/pilot_provider_smoke_aggregate.json)。

**表 6　确认性真实调用遮罩实验的预注册结果位**
...
在这些确认性单元填满之前，本文不会声称 A1、A2 或 A3 优于 B0。尤其不能因为 A2“更接近论文”就预设其更准确，也不能因为 A1 更快就预设其语义损失可接受。表 5 已经给出一个反例：更深的搜索可以带回更多证据，却不保证 reader 把证据转化为更好的终判。
...
单案例诊断进一步显示了成本梯度：B16 使用 10,820 Token、21.094 秒；full-MR 使用 129,050 Token、73.560 秒，分别是 11.93 倍和 3.49 倍。full-MR 多访问 24 条来源、多引用 4 条来源，但必要证据组召回仍与 B16 同为 1.0；它只净增一条 gold support citation，并增加三条非 gold citation。由于语义盲评尚未完成，这些数字只能说明“证据面扩大且成本显著上升”，不能把额外浏览直接记为质量收益，也不能只按 Token 选择其中之一。
...
真实调用可能受到模型版本、Provider 排队、网络、缓存预热和同时运行的后台任务影响。在线 B0 窗口没有随机对照，只能作描述性审计。超时调用可能在返回 usage 前被截断，形成右删失成本。新三臂诊断虽固定了 packet 和隔离数据库，但只有一个预选样本、每臂一次运行，执行顺序没有随机化，也没有重复估计 Provider 排队波动；它的直连非流式时延不能替代线上服务时延。未来实验将使用相同 cutoff、交替/随机 arm 顺序、重复采样和分段 ledger，避免把部署时段或请求顺序差异当作算法差异。
...
47 题的 bootstrap 区间较宽，且没有多重比较校正。三臂 Provider 诊断的独立样本数为 1，不能给出方差、置信区间、显著性或总体胜率；即使对同一调用重复采样，也不能扩大独立样本数。未来报告以调用或群为 cluster，并同时给出效应量与区间，不把 p 值当作实际价值。高级语义类别的正式样本数由后续未污染 pilot 的方差与 power 分析确定。
...
单案例诊断把问题进一步定位到了“搜索控制与最终 synthesis 的接口”。full-MR 的确找到 B16 漏掉的后续行为证据，但八轮 agent 调用没有主动收敛，最后六次工具调用没有新增 source key，最终仍未形成更直接的群内释义。这不是“模型上下文还不够大”的证据；更直接的假设是控制器缺少边际证据收益停止条件，而最终 reader 没有被要求逐项裁决必要跨情节连接、目标释义和必须保留的更强措辞疑虑。
...
第一，在这一个单群、47 题的候选检索集上，Harrier 相对 MiniLM 有明确优势，并在 k=20 时比 BM25 找到更多问题和更多证据；但 BM25 的 MRR 与 Harrier 无可靠差异，词面基线仍应保留。第二，没有证据把 B0 认定为合格语义方案：54 次线上调用全部止于 host prefetch、没有一次图升级，按“缺失 usage 计测得零”的 54-run 口径仍平均记录约 2.33 万 Token，并有 9.26% 协议/超时失败和 80 秒级 p95；但这些运行没有人工语义标签，不能据此计算准确率。第三，单案例三臂协议已经可运行并形成完整成本账本，却只显示了巨大的成本梯度：full-MR 相对 B16 多找到了证据，花费 11.93 倍 Token 和 3.49 倍受控总时延，最终却没有把新增证据转化为更直接的语义终判。该案例没有独立盲评，不能据此宣称 B16 更优，但足以说明“更深遍历自然带来更好回答”不能被当作默认假设或质量保证。第四，既有高级语义和反馈证据仍有严重边界：“好女孩”旧 rubric 没测核心语义，后续深挖也只发现候选而未确认核心边；反馈改善只发生在一个手选行为案例。
...
- [单案例三臂 Provider 诊断聚合](data/pilot_provider_smoke_aggregate.json)
...
- 单案例三臂诊断中，cache/B16/full-MR 均完成且账本完整；full-MR 扩大了证据面，但以 B16 的 11.93 倍 Token 和 3.49 倍受控总时延仍未形成更直接的终判。
