...
0.10.0 的状态。“结构完成”表示机制已进入核心代码和回归测试，不等于已经复现论文
全部基准分数。
...
| 可塑局部语义图 | 工程扩展完成 | LLM 注册/修订动态关系类型；关联边支持新增、强化、抑制、休眠、废弃和节点可逆合并；身份节点被宿主禁止 | 关系归一化、容量参数和长期漂移仍需 canary；不能把局部语义边当作确定性身份事实 |
