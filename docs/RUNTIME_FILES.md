...
本清单定义 0.18.0 插件热部署的源内边界。它只列相对路径和规则；最终部署时的逐文件
SHA-256 清单应在代码冻结后生成到 Git 忽略的 staging 目录，不能提交真实配置、数据库或
服务器快照。
...
| `metadata.yaml` | AstrBot 插件元数据与公开版本 |
...
`mr_memory/*.py` 在 0.18.0 新增的分层模块包括：
...
热部署必须以整个必需文件集合为单位，随后比较本地和远端冻结 hash 清单。schema 15→16
