...
0.16.2 让增量整理与真实反馈都能维护按群隔离的可塑关联图。局部梗义、反话和委婉语
...
等待 15 秒合并，每次最多六条。第一段只向独立模型提供目标回答、后续消息和有界 trace，
判断它是否真在评价该回答；普通聊天到此结束。只有可归因反馈才进入第二段，检索当前群的
既有假设与可塑图关联并生成有限修改计划。宿主继续校验 proposal、群、发送者、时间、证据
