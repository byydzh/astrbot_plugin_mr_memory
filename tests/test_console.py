    ...
    async def test_read_manage_and_reload(self):
        with scratch_directory() as directory:
            ...
            with patch("mr_memory.console.request", SimpleNamespace(query={}, json=AsyncMock(return_value={"account_id": "100", "alias": "合成别名"}))):
                ...
                reset = AsyncMock(return_value={"status": "cleared", "message": "已开始新会话"})
                console.conversation_repair = SimpleNamespace(reset_main_conversation=reset)
                self.assertEqual((await console.reset_conversation("synthetic"))["status"], "cleared")
                reset.assert_awaited_once_with(scope)
                self.assertEqual((await console.scope_overview("synthetic"))["counts"]["messages"], 102)
                with self.assertRaises(FileNotFoundError):
                    await console.get_store("../synthetic")
            successor = Console(plugin)
            self.assertEqual(len(context.registered_web_apis), 13)
            console.close()
            self.assertEqual(len(context.registered_web_apis), 13)
            successor.close()
